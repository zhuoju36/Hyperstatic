...
from structengpy.fe_model.quad import Membrane4
...
class Model:
    ...
    def add_node(self,name:str,x:float,y:float,z:float,check_dup=False,tol=1e-6)->int:
        """
        add node to model
        ...
        """
        node=Node(x,y,z,name)
        if check_dup:
            res=[a for a in self.nodes.values() if np.linalg.norm(a.loc-np.array([x,y,z]))<1e-6]
        else:
            res=[]
        if res==[]:
...
#            self.__f[node.hid*6:node.hid*6+6,0]=np.dot(Tt,node.fn) 
            fn=self.__loadcase[casename].get_nodal_load_vec(node.name)
            fn_=np.dot(Tt,fn)
            k=0
